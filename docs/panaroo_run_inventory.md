...
The lollipop plots each strain as a connected line across **five** nodes; values are the mean number of genes shared between the run's query samples and the chosen RefSeq.
...
|---|---|---|
| `kp_epidemic` | One row per major CG (`n_samples ≥ --min-group-size`) within a KP sublineage Panaroo run. `b.i` is the CG's own best RefSeq; `b.ii` is its K-locus subgroup mean. |
| `kp_epidemic_sl` | One row per KP sublineage Panaroo run. `b.i` and `b.ii` are weighted means across **all** top-level CG subgroups in the run, including the `other` bucket of small CGs (so the SL row truly represents the whole sublineage, not just its big CGs). |
