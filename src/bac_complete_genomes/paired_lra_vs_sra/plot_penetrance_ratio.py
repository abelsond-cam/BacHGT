#!/usr/bin/env python3
"""Horizontal bar chart of paired LR/SR penetrance ratio per feature.
...
For each row computes the 95% CI on the paired prevalence ratio using the
...
with the 2×2 cells recovered from the wide schema:
    ...
    a + b = n_lr * lr_penetrance
    a + c = n_lr * sr_penetrance
...
def _paired_log_ratio_ci(df: pd.DataFrame, z: float = 1.96) -> pd.DataFrame:
    ...
    p_lr = out["lr_penetrance"].astype(float)
    p_sr = out["sr_penetrance"].astype(float)
    ...
    ratio = out["lr_sr_penetrance_ratio"].astype(float)
    out["ratio"] = ratio
...
def _plot_one(df: pd.DataFrame, out_png: Path, title: str) -> None:
    ...
    ax.set_xlabel("LR / SR penetrance ratio (95% CI)")
    ax.set_title(title)
...
def main() -> None:
    ...
    _plot_one(
        ...
        f"LR vs SR prevalence ratio — Kleborate ({args.cohort})",
    )
    _plot_one(
        isescan,
        args.output_dir / "lra_vs_sr_prevalence_isescan_ratio.png",
        f"LR vs SR prevalence ratio — ISEScan ({args.cohort})",
    )
