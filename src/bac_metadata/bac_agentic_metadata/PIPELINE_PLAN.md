...
## 6b. Current status & results

**Moved to [`PROGRESS_REPORT.md`](PROGRESS_REPORT.md).** Current status, every measured number
(sizing, grading + adjudication, finding + adjudication + secondary-accession lift + abstention
rescue, backfill targeting), the ground-truth-quality finding, and the forward plan now live there —
kept in one place so the figures cannot drift across docs. This file stays the framework/why
(architecture, attribute-spec model, ground-truth discipline, staged build).
