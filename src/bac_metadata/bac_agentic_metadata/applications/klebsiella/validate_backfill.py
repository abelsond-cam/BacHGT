"""Validate method-(a) whole-project backfill for the four sample-level fields (train+val).

The grader proposes a single whole-project value for ``country`` / ``collection_date`` /
``isolation_source`` / ``host`` when the paper supports one (method-(a)). Here we measure how well
...
value's correctness here (that needs the per-sample ``metadata_v2`` table — deferred with method-(b)).
What we CAN measure is **targeting / recall**: where ENA left a field largely blank (low ``_pre``),
did method-(a) propose a whole-project value, and does that align with where curation actually lifted
completeness? Fillable accessions with no whole-project proposal are the method-(b) per-sample
backlog (deferred).
...
def _proposals() -> pd.DataFrame:
    """Per-accession method-(a) proposal flags + values from the grades TSV (train+val)."""
    g = pd.read_csv(DATA_DIR / "study_grades.tsv", sep="\t", dtype=str).fillna("")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Validate method-(a) backfill targeting (Klebsiella).")
    ...
    md = ["# Stage 2 — method-(a) backfill targeting vs parsed_per_project (train+val)\n"]
    md.append(f"Accessions with both a grade and a parsed_per_project row: **{len(df)}**.\n")
    md.append("`needs` = ENA non-null `<field>_pre` < threshold (a real gap). `covered` = method-(a) "
    ...
    md.append("| field | needs backfill | covered by method-a | residual (method-b) | redundant | "
              "recall vs curation |")
    md.append("|---|---|---|---|---|---|")
    ...
    for f in FIELDS:
        ...
        # recall: of accessions curation actually improved, how many did method-a also flag?
    ...
    md.append("\n## Reading it\n")
    md.append("- **covered**: method-(a) supplies a whole-project value for a field ENA left blank — "
              "ready to apply (value correctness still to be checked vs metadata_v2).")
    md.append("- **residual (method-b)**: a real gap with no single whole-project value — the deferred "
              "per-sample-table backlog.")
    md.append("- **recall vs curation**: where curation demonstrably filled the field, did method-(a) "
              "also flag it? Low recall ⇒ much of the win needs method-(b).")

    (DATA_DIR / "backfill_validation_report.md").write_text("\n".join(md) + "\n")
    pd.DataFrame(rows).sort_values(["field", "study_accession"]).to_csv(
        DATA_DIR / "backfill_validation_report.tsv", sep="\t", index=False)
