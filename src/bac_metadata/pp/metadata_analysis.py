...
AMR_STUDY_STACK_ORDER = ["AMR", "AMR plus control", "Surveillance", "Missing"]

# Study setting colors and stack order
STUDY_SETTING_COLORS = {"Hospital": "steelblue", "Mixed": "lightblue", "Community": "darkgreen", "Missing": "gray"}

STUDY_SETTING_STACK_ORDER = ["Hospital", "Mixed", "Community", "Missing"]
...
def plot_all_kleb_species_histogram(df: pd.DataFrame, output_path: Path) -> None:
    ...
    if "is_kpsc" in df.columns:
        ...
        ax.barh(range(len(top_10_sorted)), top_10_sorted.values, color="steelblue", edgecolor="black", linewidth=0.5)
        ...
        for i, (_idx, val) in enumerate(top_10_sorted.items()):
    ...
    species_kpsc_map = (
        df.groupby("species")[kpsc_col].apply(lambda x: x.mode()[0] if len(x.mode()) > 0 else False).to_dict()
    )
    ...
    colors = ["grey" if not species_kpsc_map.get(sp, False) else "steelblue" for sp in combined_species.index]
    ...
    ax.barh(range(len(combined_species)), combined_species.values, color=colors, edgecolor="black", linewidth=0.5)
    ...
    for i, (_idx, val) in enumerate(combined_species.items()):
        ax.text(val, i, f" {val:,}", va="center", fontsize=8, fontweight="bold")
    ...
    ax.text(
        ...
        ha="center",
        va="center",
        bbox={"boxstyle": "round", "facecolor": "steelblue", "alpha": 0.3},
    )
    ax.text(
        0.8,
        0.2,
        f"n={total_kpsc_false:,}",
        transform=ax.transAxes,
        fontsize=11,
        fontweight="bold",
        ha="center",
        va="center",
        bbox={"boxstyle": "round", "facecolor": "grey", "alpha": 0.3},
    )
...
def plot_all_kleb_metadata_counts(
    df: pd.DataFrame, output_path: Path, x_axis: str, stack_by: str, top_n: int = 7, title: str = None
) -> None:
    ...
    for i, (_idx, row) in enumerate(crosstab.iterrows()):
        total = row.sum()
        if total > 0:
...
def plot_all_kleb_metadata_proportions(
    df: pd.DataFrame, output_path: Path, x_axis: str, stack_by: str, top_n: int = None
) -> None:
    ...
    ax.set_title(f"{stack_by} distribution by {x_axis}", fontsize=13, fontweight="bold", pad=15)
    ax.set_ylim(0, 1.3)
...
def plot_kpsc_subspecies_cumulative_histogram(df: pd.DataFrame, output_path: Path, group_label: str) -> None:
    ...
    ax1.bar(
        range(len(subspecies_percentages)),
        subspecies_percentages.values,
        color=colors,
        edgecolor="black",
        linewidth=0.3,
        alpha=0.7,
    ...
    ax2 = ax1.twinx()
    ax2.plot(range(len(cumulative_pct)), smoothed_pct, color="red", linewidth=2.5, label="Cumulative % (smoothed)")
    ...
    ax1.set_title(f"K. pneumoniae {group_label} Distribution", fontsize=13, fontweight="bold", pad=15)
...
def plot_kpsc_subspecies_proportions(
    df: pd.DataFrame, output_path: Path, stack_by: str, group_label: str, subspecies_order: list = None, top_n: int = 10
) -> list:
    ...
    if subspecies_order is None:
        ...
        remaining_subspecies = (
            subspecies_counts_all.iloc[top_n:].index.tolist() if len(subspecies_counts_all) > top_n else []
        )
        ...
        top_sorted = sorted(
            [k for k in sample_counts_dict.keys() if k != "Other"], key=lambda x: sample_counts_dict[x], reverse=True
        )
        ...
        for subspecies_group_name in subspecies_order:
            if subspecies_group_name == "Other":
                ...
                try:
                    subspecies_val = (
                        int(subspecies_group_name) if subspecies_group_name.isdigit() else subspecies_group_name
                    )
        ...
        for subspecies_group_name in subspecies_order:
            if subspecies_group_name in subspecies_group_mapping:
                ...
                proportions_data[subspecies_group_name] = calc_props(
                    subspecies_list, subspecies_group_name, stack_categories
                )

                if subspecies_group_name == "Other":
                    sample_counts_dict[subspecies_group_name] = (
                        subspecies_counts_all[subspecies_list].sum() if subspecies_list else 0
                    )
                else:
    ...
    ax.set_ylabel("Proportion of all samples", fontsize=11, fontweight="bold")
    ax.set_title(f"{stack_by} distribution by {group_label}", fontsize=13, fontweight="bold", pad=15)
    ax.set_ylim(0, 1.5)
    ax.set_yticks([0, 0.25, 0.5, 0.75, 1.0])
...
def plot_kpsc_subspecies_counts(df: pd.DataFrame, output_path: Path, stack_by: str, group_label: str) -> None:
    ...
    filtered["subspecies_label"] = filtered["subspecies_group"].apply(
        lambda x: str(x) if x in top_30.index else "Other"
    )
    ...
    # Create title
    title_map = {"amr_study": "AMR Study Type", "study_setting": "Study Setting"}
...
def plot_kpsc_subspecies_by_K_locus(
    df: pd.DataFrame, output_path: Path, subspecies_descriptor: str = "Sublineage", top_n: int = 10
) -> None:
    ...
    descriptor_mapping = {"Sublineage": ("Sublineage", "Sub-Lineage"), "Clonal group": ("Clonal group", "Clonal Group")}

    if subspecies_descriptor not in descriptor_mapping:
        raise ValueError(
            f"Invalid subspecies_descriptor '{subspecies_descriptor}'. Must be 'Sublineage' or 'Clonal group'"
        )
    ...
    for bar_idx, group_name in enumerate(groups_to_plot):
        ...
        for locus_label, proportion in sorted_loci:
            # Plot segment
            ax.bar(
                bar_idx, proportion, bottom=current_bottom, color=bar_color, edgecolor="black", linewidth=1.0, alpha=0.8
            )
            ...
            if proportion >= 0.05:
                ...
                ax.text(
                    ...
                    fontweight="bold",
                    color="white",
                    bbox={"boxstyle": "round,pad=0.3", "facecolor": "black", "alpha": 0.6, "edgecolor": "white", "linewidth": 0.5},
                )
    ...
    ax.set_xlabel(f"{group_label}", fontsize=11, fontweight="bold")
    ax.set_ylabel("Proportion of all samples", fontsize=11, fontweight="bold")
    ax.set_title(f"K_locus by {group_label}", fontsize=13, fontweight="bold", pad=15)
    ax.set_ylim(0, 1.1)
...
def plot_kpsc_subspecies_by_isolation_source_by_region(
    df: pd.DataFrame, output_path: Path, subspecies_order: list, group_label: str, top_n: int = 10
) -> None:
    ...
    filtered = df[
        df["subspecies_group"].notna() & df["isolation_source_category"].notna() & df["region"].notna()
    ].copy()
        ...
        for subspecies_group_name in subspecies_order:
            ...
            if count_in_region < 100:
                proportions_data[subspecies_group_name] = pd.Series(
                    [0.0] * len(top_4_categories), index=top_4_categories, name=subspecies_group_name
                )
            else:
...
def main():
    ...
    parser.add_argument("--top-n-st", type=int, default=10, help="Number of top STs to show individually (default: 10)")
    parser.add_argument(
    ...
    try:
        ...
        if args.plot_all_klebsiella_species:
            ...
            if "kpsc_final_list" not in df.columns:
                ...
                df_kpsc = df[(df["species"] == "Klebsiella pneumoniae") & df["kpsc_final_list"]].copy()
        ...
        if group_col not in df_kpsc.columns:
            print(
                f"Error: Column '{group_col}' not found in data. Cannot use subspecies={args.subspecies}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        plot_kpsc_subspecies_cumulative_histogram(
            df_kpsc_sub, output_dir / f"kpsc_{subspecies_token}_histogram.png", group_label
        )

        # Subspecies-based proportion plots (create subspecies order first, then reuse)
        subspecies_order = plot_kpsc_subspecies_proportions(
            df_kpsc_sub,
            output_dir / f"isolation_source_by_{subspecies_token}.png",
            stack_by="isolation_source_category",
            group_label=group_label,
            top_n=args.top_n_st,
        )

        plot_kpsc_subspecies_proportions(
            df_kpsc_sub,
            output_dir / f"region_by_{subspecies_token}.png",
            stack_by="region",
            group_label=group_label,
            subspecies_order=subspecies_order,
            top_n=args.top_n_st,
        )

        # K_locus plots for both Sublineage and Clonal group (independent of subspecies dimension)
        plot_kpsc_subspecies_by_K_locus(
            df_kpsc_sub,
            output_dir / "k_locus_by_sublineage.png",
            subspecies_descriptor="Sublineage",
            top_n=args.top_n_st,
        )
        plot_kpsc_subspecies_by_K_locus(
            df_kpsc_sub,
            output_dir / "k_locus_by_clonal_group.png",
            subspecies_descriptor="Clonal group",
            top_n=args.top_n_st,
        )

        # Specialized subspecies proportion plots (use subspecies_order from isolation_source plot)
        plot_kpsc_subspecies_by_isolation_source_by_region(
            df_kpsc_sub,
            output_dir / f"isolation_source_by_{subspecies_token}_by_region.png",
            subspecies_order=subspecies_order,
            group_label=group_label,
            top_n=args.top_n_st,
        )
        ...
        plot_all_kleb_metadata_proportions(
            df, output_dir / "region_by_isolation_source.png", x_axis="isolation_source_category", stack_by="region"
        )

        # Subspecies-based count plots
        plot_kpsc_subspecies_counts(
            df_kpsc_sub,
            output_dir / f"amr_study_by_{subspecies_token}.png",
            stack_by="amr_study",
            group_label=group_label,
        )
        plot_kpsc_subspecies_counts(
            df_kpsc_sub,
            output_dir / f"study_setting_by_{subspecies_token}.png",
            stack_by="study_setting",
            group_label=group_label,
        )

        # Comparison of two categorical variables (not ST) - uses full dataset df
        plot_all_kleb_metadata_counts(
            df,
            output_dir / "amr_study_by_isolation_source.png",
            x_axis="isolation_source_category",
            stack_by="amr_study",
            top_n=12,
        )
        plot_all_kleb_metadata_counts(
            df_kpsc,
            output_dir / "study_setting_by_isolation_source.png",
            x_axis="isolation_source_category",
            stack_by="study_setting",
            top_n=12,
        )
        plot_all_kleb_metadata_counts(
            df_kpsc, output_dir / "amr_study_by_study_setting.png", x_axis="study_setting", stack_by="amr_study"
        )
        # Isolation source category by host category
        plot_all_kleb_metadata_counts(
            df_kpsc,
            output_dir / "host_catetory_by_isolation_source.png",
            x_axis="isolation_source_category",
            stack_by="host_catetory",
            top_n=12,
        )
    ...
    except Exception as e:  # noqa: BLE001 — top-level: restore stdout no matter how plotting fails
