...
Copied from BacPredict ``bac_pyseer/kleb_iso_source/resolve_snippy_paths.py`` (separate repo)
— the resolution logic is cohort-agnostic, so it is reused verbatim here.

The cohort is keyed by metadata_v2 ``Sample`` (a biosample ``SAMN*/SAME*/SAMD*`` for
short-read rows, or a versioned ``GCF_/GCA_`` accession for long-read rows). The variant
calls, however, live in two trees keyed differently:
...
Both sources point at the **raw** VCF on purpose: a single, uniform re-filter from raw across
every sample, so the native filtered ``snps.vcf`` retained by ``snippy_ncbi`` is intentionally
bypassed here (it is used only as filter-fidelity ground truth elsewhere).
...
def _load_sample_list(sample_csv: Path | None, metadata: pd.DataFrame, all_kpsc: bool) -> pd.Series:
    ...
    Either the ``Sample`` column of a cohort/split CSV, or — with ``all_kpsc`` — every
    ``kpsc_final_list == True`` row of metadata_v2.
    """
    if all_kpsc:
...
def main(argv: list[str] | None = None) -> None:
    ...
    src.add_argument("--sample-csv", type=Path, help="Cohort/split CSV with a 'Sample' column.")
    src.add_argument("--all-kpsc", action="store_true", help="Use every kpsc_final_list==True Sample.")
