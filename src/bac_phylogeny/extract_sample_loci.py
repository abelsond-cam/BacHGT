...
Copied from BacPredict ``bac_pyseer/kleb_iso_source/extract_sample_loci.py`` (separate repo)
— the extraction is cohort-agnostic and the per-sample cache is shared across cohorts, so it
...
Three per-call fields drive it:
...
- ``FMT/DP >= --min-dp``  (default 3)   — read depth in this sample. Kept at 3 (not 10) so
  ...
  noise is removed downstream by the cohort-level "present in >X% of samples" locus filter.
...
incrementally. Output file: ``<cache>/<Sample>.loci.tsv.gz`` (header ``POS\\tREF\\tALT``).
Per-sample failures are recorded under ``<cache>/_failures/<Sample>.txt`` (no cross-task races).
"""
