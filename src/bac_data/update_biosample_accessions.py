...
def query_ncbi_datasets(accessions: list[str], temp_dir: Path, retry_on_error: bool = True) -> dict[str, str]:
    ...
    try:
        ...
        dataformat_cmd = [str(DATAFORMAT_BIN), "tsv", "genome", "--fields", "accession,assminfo-biosample-accession"]

        # Run datasets and pipe to dataformat
        with open(output_file, "w") as outf:
            datasets_proc = subprocess.Popen(datasets_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

            dataformat_proc = subprocess.Popen(
                dataformat_cmd, stdin=datasets_proc.stdout, stdout=outf, stderr=subprocess.PIPE
            )
            ...
            if datasets_return != 0:
                ...
                print(
                    "Continuing with partial results (unmapped accessions will remain as GCF_ format)...\n",
                    file=sys.stderr,
                )
        ...
        print("Please ensure datasets and dataformat are installed at:", file=sys.stderr)
        print(f"  {DATASETS_BIN}", file=sys.stderr)
        print(f"  {DATAFORMAT_BIN}", file=sys.stderr)
        sys.exit(1)
    except Exception as e:  # noqa: BLE001 — top-level fallback: surface any subprocess/parse failure and abort
    ...
    try:
        ...
        if not output_file.exists() or output_file.stat().st_size == 0:
            print("WARNING: No output from NCBI datasets command", file=sys.stderr)

            # If retry is enabled, try to recover
            if retry_on_error:
                if len(accessions) > 10:
                    # Split in half and retry
                    print("Retrying with smaller sub-batches to isolate problematic accessions...", file=sys.stderr)
                    ...
                    if mapping:
                        print(
                            f"  ✓ Successfully mapped {len(mapping)} out of {len(accessions)} accessions",
                            file=sys.stderr,
                        )
                    return mapping
                else:
                ...
                print(
                    "Retry disabled - all accessions in this batch will remain unchanged as GCF_ format",
                    file=sys.stderr,
                )
                return mapping
        ...
        if len(df) == 0:
            print("WARNING: Empty results from NCBI", file=sys.stderr)
    ...
    except Exception as e:  # noqa: BLE001 — resilience: warn and return partial mapping on any parse failure
...
def main():
    ...
    parser.add_argument("--dry-run", action="store_true", help="Test with first 100 rows without saving changes")
    ...
    if mapping:
        ...
        for assembly, biosample in list(mapping.items())[:10]:
