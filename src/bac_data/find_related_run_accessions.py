...
def filter_short_reads(runs: list[dict]) -> list[dict]:
    ...
    return [
        r for r in runs
        if r.get("instrument_platform", "").upper() in SHORT_READ_PLATFORMS
    ]
...
def resolve_gcf_to_biosample(
    ...
    print(
        f"\n[resolve_gcf] Resolving {len(gcf_accessions):,} GCF accessions "
        f"via NCBI Datasets API ({key_status})",
        file=sys.stderr,
    )
    print(
    ...
    with ThreadPoolExecutor(max_workers=actual_workers) as pool:
        future_to_batch = {
            pool.submit(query_ncbi_datasets_batch, batch): batch
    ...
    print(f"[resolve_gcf]   Successfully resolved        : {n_resolved:,}  ({100*n_resolved/n_total:.1f}%)", file=sys.stderr)
    ...
    _ncbi_cols = ["gcf", "biosample", "bioproject_accession", "release_date",
                  "submitter", "sequencing_tech", "assembly_method", "bioproject_title"]
    enriched_df = (
        pd.DataFrame(all_records, columns=_ncbi_cols)
        if all_records
        else pd.DataFrame(columns=_ncbi_cols)
    )

    # Per-field non-empty counts — shows how complete the NCBI enrichment is
    if not enriched_df.empty:
        print(f"[resolve_gcf]   Non-empty values per enriched field:", file=sys.stderr)
        for field in ["bioproject_accession", "release_date", "submitter",
                      "sequencing_tech", "assembly_method", "bioproject_title"]:
            n_filled = int((enriched_df[field].astype(str).str.strip() != "").sum())
            pct = 100 * n_filled / len(enriched_df)
            print(f"[resolve_gcf]     {field:<22}: {n_filled:>5,}  ({pct:5.1f}%)",
                  file=sys.stderr)
...
def run_long_read_pipeline(
    accessions: list[str], output_dir: Path, dataset_label: str,
...
def run_short_read_pipeline(
    ...
    if write_outputs:
        pd.DataFrame(
            list(gcf_to_biosample.items()), columns=["gcf_accession", "biosample_accession"]
        ).to_csv(out_gcf_biosample, sep="\t", index=False)
        print(f"[{dataset_label}] GCF→BioSample map ({len(gcf_to_biosample):,} entries) written to:\n    {out_gcf_biosample}", file=sys.stderr)
        ...
        print(f"[{dataset_label}] Unresolved GCF accessions ({len(unresolved_gcfs):,}) written to:\n    {out_unresolved}", file=sys.stderr)
    ...
    if "sample_accession" in df.columns:
        df.insert(0, "gcf_accession",
                  df["sample_accession"].map(biosample_to_gcf)
    ...
    if write_outputs:
        ...
        summary_cols = [
            c for c in [
                "gcf_accession", "sample_accession", "secondary_sample_accession",
                "instrument_platform", "instrument_model",
                "run_accession", "library_strategy", "library_layout",
                "read_count", "base_count", "fastq_ftp", "first_public",
            ]
            if c in df.columns
        ]
        df[summary_cols].to_csv(out_summary, sep="\t", index=False)
    ...
    matched_via_secondary_only = sorted(
        a for a in biosample_accessions
        if a in samples_with_sr and a not in primary_vals
    )
    if write_outputs:
        Path(out_no_sr).write_text("\n".join(no_sr) + "\n")
        print(
    ...
    if "base_count" in df.columns:
        ...
        print(f"  base_count — min: {bc.min():,.0f}  median: {bc.median():,.0f}  "
              f"mean: {bc.mean():,.0f}  max: {bc.max():,.0f}")
    if "read_count" in df.columns:
        rc = df["read_count"].dropna()
        print(f"  read_count — min: {rc.min():,.0f}  median: {rc.median():,.0f}  "
    ...
    print(f"  BioSamples with ≥1 short-read run   : {n_queried_with_sr:,}  "
...
def report_long_read_overlaps_with_refseq(
    ...
    if long_read_df.empty:
        print(f"  No long-read runs to cross-check.")
        return

    lr_primary = (
        set(long_read_df["sample_accession"].dropna().astype(str).str.strip())
        if "sample_accession" in long_read_df.columns else set()
    )
    lr_secondary = (
        set(long_read_df["secondary_sample_accession"].dropna().astype(str).str.strip())
        if "secondary_sample_accession" in long_read_df.columns else set()
    )
    ...
    rs_gcf = (
        ...
        if GCF_ACCESSION_COL in refseq_meta.columns else set()
    )
    ...
    print(f"  → These samples already have complete RefSeq genomes.")
...
def report_short_read_overlaps_with_nonrefseq(
    ...
    if short_read_df.empty:
        print(f"  No short-read runs to cross-check.")
        return

    sr_primary = (
        set(short_read_df["sample_accession"].dropna().astype(str).str.strip())
        if "sample_accession" in short_read_df.columns else set()
    )
    ...
    mask = short_read_df["sample_accession"].astype(str).isin(overlap_accs) \
    ...
    print(f"  → These RefSeq assembly short reads are already tracked as non-RefSeq samples.")
    print(f"    They are strong candidates for the related_short_read_accession link column.")
...
def print_clonal_group_summary(
    ...
    cg_sr = (
        sr_df[sr_df["gcf"].isin(gcfs_with_sr)]
        .groupby(cg_col).size()
        .rename("n_refseq_with_sr")
    )
    ...
    for _, row in summary.iterrows():
        ...
        print(f"  {str(row[cg_col]):<20}  {row['n_total']:>8,}  {row['n_is_refseq']:>11,}  "
...
def _print_cg_run_summary(
    ...
    if "est_coverage_5_5Mb" in meta.columns:
        is_appended_meta = (
            pd.to_numeric(meta["est_coverage_5_5Mb"], errors="coerce").notna()
            & (meta[IS_REFSEQ_COL] == False)
        )
    ...
    if kpsc_col in meta.columns:
        src = meta[(meta[kpsc_col] == True) & ~is_appended_meta].copy()
    else:
        src = meta[~is_appended_meta].copy()

    def _agg(g):
        is_rs  = g[IS_REFSEQ_COL] == True
        is_nrs = g[IS_REFSEQ_COL] == False
        row = {"n_total": len(g), "n_is_refseq": int(is_rs.sum())}
        if has_sr:
            row["n_refseq_with_sr"] = int(
                (is_rs & g["related_sr_accession"].notna()).sum()
            )
        if has_lr:
            link = g["related_lr_accession"].astype(str)
            row["n_nonrefseq_with_suff_lr"] = int(
                (is_nrs & link.isin(suff_lr_run_accs)).sum()
            )
    ...
    print(f"  {sep*w}" + "".join(f"  {sep*18}" for _ in cols) + (f"  {sep*10}" if has_sr else ""), file=sys.stderr)
...
def _build_side_dataframe(
    ...
    *,
    ...
    suff = (
        (base_count >= MIN_BASES_FOR_ASSEMBLY).values
        if role == "long_read"
        else [pd.NA] * n
    )
...
def _apply_run_info(
    ...
    _current_cols = (
        "related_lr_accession", "related_sr_accession",
        "assembly_sequencing_tech", "assembly_method",
        "assembly_bioproject_accession", "assembly_release_date",
        "assembly_submitter", "assembly_bioproject_title",
        "est_coverage_5_5Mb", "sufficient_for_hybrid",
    )
    ...
    cols_to_drop = [
        ...
        if c.startswith(_legacy_prefixes) or c in _current_cols
    ]
    if cols_to_drop:
        meta = meta.drop(columns=cols_to_drop)
        if verbose:
            print(f"  Cleared {len(cols_to_drop)} pre-existing run columns "
                  f"(re-running this script repopulates them)", file=sys.stderr)
            print(f"  Snapshot of prior run-accessions for dedup: "
                  f"{len(prior_sr_accs):,} SR, {len(prior_lr_accs):,} LR", file=sys.stderr)

    n_refseq    = int((meta[IS_REFSEQ_COL] == True).sum())
    n_nonrefseq = int((meta[IS_REFSEQ_COL] == False).sum())
    ...
    if enriched_ncbi_df is not None and not enriched_ncbi_df.empty and "gcf" in enriched_ncbi_df.columns:
        ...
        ncbi_renamed = enriched_ncbi_df.rename(columns={
    ...
    sr_best     = _select_best_runs(sr_filtered, "gcf_accession") if sr_filtered is not None and not sr_filtered.empty else None
    ...
    if verbose:
        ...
        print(f"  │  NCBI Datasets resolved GCFs (BioSample + assembly metadata)", file=sys.stderr)
        print(f"  │      Resolved                                  : {n_resolved:,} of {n_refseq:,} ({100*n_resolved/max(n_refseq,1):.1f}%)", file=sys.stderr)
        for col in ["assembly_sequencing_tech", "assembly_method",
                    "assembly_bioproject_accession", "assembly_release_date",
                    "assembly_submitter", "assembly_bioproject_title"]:
        ...
        print(f"  │  Short-read runs found at these BioSamples   (join key: GCF):", file=sys.stderr)
        print(f"  │      Total runs found                          : {sr_total:,}", file=sys.stderr)
        print(f"  │      After library filter (WGS+PAIRED)         : {sr_filt_n:,}  ({sr_total - sr_filt_n:,} dropped)", file=sys.stderr)
        print(f"  │      Best run per BioSample (unique GCFs)      : {sr_best_n:,}  ({sr_filt_n - sr_best_n:,} dropped)", file=sys.stderr)
    ...
    lr_best     = _select_best_runs(lr_filtered, SAMPLE_ACCESSION_COL) if lr_filtered is not None and not lr_filtered.empty else None
    lr_best_n   = len(lr_best) if lr_best is not None else 0
    lr_suff     = int(lr_best["sufficient_for_hybrid"].fillna(False).astype(bool).sum()) if lr_best is not None and not lr_best.empty else 0
    ...
    if verbose:
        ...
        print(f"  │", file=sys.stderr)
        print(f"  │  Long-read runs found at these BioSamples   (join key: BioSample):", file=sys.stderr)
        print(f"  │      Total runs found                          : {lr_total:,}", file=sys.stderr)
        print(f"  │      After library filter (WGS)                : {lr_filt_n:,}  ({lr_total - lr_filt_n:,} dropped)", file=sys.stderr)
        print(f"  │      Best run per BioSample (unique)           : {lr_best_n:,}  ({lr_filt_n - lr_best_n:,} dropped)", file=sys.stderr)
        print(f"  │      → 'related_lr_accession' set on {lr_existing_filled:,} existing non-RefSeq rows", file=sys.stderr)
        print(f"  │      → {lr_best_n:,} LR runs will be written to RELATED_LR_CSV ({lr_suff:,} sufficient for hybrid; subject to dedup)", file=sys.stderr)
        print("  └────────────────────────────────────────────────────────────────────", file=sys.stderr)
    ...
    refseq_mask = meta[IS_REFSEQ_COL] == True
    ...
    atb_best     = _select_best_runs(atb_filtered, SAMPLE_ACCESSION_COL) if atb_filtered is not None and not atb_filtered.empty else None
    atb_best_n   = len(atb_best) if atb_best is not None else 0
    atb_suff     = 0
    if atb_best is not None and not atb_best.empty:
        atb_suff = int(
            (pd.to_numeric(atb_best.get("base_count"), errors="coerce") >= MIN_BASES_FOR_ASSEMBLY)
            .fillna(False).sum()
        )

    if verbose and atb_total > 0:
        print("\n  ┌─ (iii) ATB increment BioSamples → ENA long reads ─────────────────", file=sys.stderr)
        print(f"  │  ATB BioSamples are NOT in main metadata; runs land in RELATED_LR_CSV", file=sys.stderr)
        print(f"  │      with source_section='atb_long_read' and linked_curated_sample=NaN.", file=sys.stderr)
        print(f"  │", file=sys.stderr)
        print(f"  │  Long-read runs found at these BioSamples:", file=sys.stderr)
        print(f"  │      Total runs found                          : {atb_total:,}", file=sys.stderr)
        print(f"  │      After library filter (WGS)                : {atb_filt_n:,}  ({atb_total - atb_filt_n:,} dropped)", file=sys.stderr)
        print(f"  │      Best run per BioSample (unique)           : {atb_best_n:,}  ({atb_filt_n - atb_best_n:,} dropped)", file=sys.stderr)
        print(f"  │      → {atb_best_n:,} ATB LR runs will be added to RELATED_LR_CSV ({atb_suff:,} sufficient for hybrid; subject to dedup)", file=sys.stderr)
        print("  └────────────────────────────────────────────────────────────────────", file=sys.stderr)
    ...
    if sr_best is not None and not sr_best.empty:
        ...
        gcf_to_sample = dict(zip(rf[GCF_ACCESSION_COL].astype(str), rf["Sample"].astype(str)))
        sr_linked = sr_best["gcf_accession"].astype(str).map(gcf_to_sample)
        sr_side_df = _build_side_dataframe(
            sr_best, role="short_read",
    ...
    sr_drop = sr_side_df["run_accession"].astype(str).isin(prior_sr_accs) if sr_side_total else pd.Series([], dtype=bool)
    ...
    if verbose:
        ...
        print(f"  related_sr_accessions.csv: {sr_side_total:,} runs found, "
              f"{sr_dedup_dropped:,} dropped as already in metadata, "
              f"{n_sr_written:,} written → {RELATED_SR_CSV}", file=sys.stderr)
        print(f"  related_lr_accessions.csv: {lr_total:,} runs found "
              ...
              f"{n_lr_written:,} written → {RELATED_LR_CSV}", file=sys.stderr)
        print("═══════════════════════════════════════════════════════════", file=sys.stderr)
...
def update_metadata_with_run_info(
    ...
    for i, path in enumerate(metadata_files):
        ...
        print(f"\n── File {i+1}/{len(metadata_files)}{marker}: {os.path.basename(path)}",
              file=sys.stderr)
        ...
        meta = _apply_run_info(
            meta, lr_df, sr_df, enriched_ncbi_df, atb_lr_df=atb_lr_df, verbose=is_primary
        )
        ...
        print(f"  Final shape     : {meta.shape[0]:,} rows × {meta.shape[1]} columns",
              file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--sr-only", action="store_true",
                        help="Skip long-read ENA queries (metadata + ATB); run only the "
                             "RefSeq→BioSample→short-read pipeline")
    ...
    if args.sr_only:
        ...
        meta_lr_df = run_long_read_pipeline(
            accessions, Path(LONG_READS_OUTPUT_DIR), "metadata_non_refseq",
            write_outputs=False,
        )
    ...
    if kpsc_col in refseq_rows.columns:
        ...
        refseq_rows = refseq_rows[refseq_rows[kpsc_col] == True].copy()
    ...
    if GCF_ACCESSION_COL in refseq_rows.columns:
        ...
        print(
            f"  WARNING: '{GCF_ACCESSION_COL}' not found — extracting GCF via regex from "
            f"'{SAMPLE_ACCESSION_COL}'",
            file=sys.stderr,
        )
        gcf_accessions = (
    ...
    if not meta_lr_df.empty:
        report_long_read_overlaps_with_refseq(
            meta_lr_df, refseq_rows, "metadata_non_refseq",
            Path(LONG_READS_OUTPUT_DIR), write_outputs=False,
        )

    # Cross-check 2: short reads found for RefSeq samples vs non-refseq metadata
    if not sr_df.empty:
        report_short_read_overlaps_with_nonrefseq(
            sr_df, non_refseq, "refseq_short_reads",
            Path(SHORT_READS_OUTPUT_DIR), write_outputs=False,
        )

    # Clonal group breakdown — universe is all KPSC samples (any is_refseq)
    all_kpsc_rows = meta[meta[kpsc_col] == True].copy() if kpsc_col in meta.columns else meta
    ...
    if args.sr_only:
        ...
        if not atb_lr_df.empty:
            report_long_read_overlaps_with_refseq(
                atb_lr_df, refseq_rows, "atb_klebsiella",
                Path(ATB_LONG_READS_OUTPUT_DIR),
            )
