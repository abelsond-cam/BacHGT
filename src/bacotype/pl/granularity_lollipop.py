...
Connecting-line color indicates row type:
  ...
  orange — KP rare-lineage batch (kp_rare)
  blue   — Non-KP Klebsiella species (kp_species)
"""
...
_LINE_COLORS = {
    "kp_epidemic": "gray",
    "kp_rare": "#ff7f0e",    # orange
    "kp_species": "#1f77b4",  # steel blue
}
...
def plot_granularity_lollipop(
    ...
    *,
    ...
    # Keep only rows with all four level values
    df = df[
    ...
    for row in df.itertuples():
        levels = ["d", "c", "b", "a"]
        values = [row.shared_genes_d, row.shared_genes_c, row.shared_genes_b, row.shared_genes_a]
        ...
        if label:
            ax.text(3.05, row.shared_genes_a, label, fontsize=7, va="center", alpha=0.7)

    ax.set_ylabel("Shared genes with reference genome", fontsize=11, fontweight="bold")
    ax.set_xticks(range(4))
    ax.set_xticklabels(
        ["d: mgh78578", "c: Sublineage RefSeq", "b: CG RefSeq", "a: Per-sample RefSeq"],
        fontsize=10,
    )
    ...
    legend_elements = [
        Line2D([0], [0], marker="o", color="w", label="d: mgh78578",
