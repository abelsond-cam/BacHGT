...
table), and the honest funnel. INV2 (master ⊇ git HEAD) is exercised against a throwaway git repo. Paths are
resolved through ``RunPaths`` (the run_progress/<tag>/<stage>/ layout), the same authority the engine uses.
"""
...
def _rp(tmp_path: Path, tag: str = "train") -> RunPaths:
    return RunPaths(tmp_path / "data", tag)
...
def test_inv3_fill_detects_blank_final_cell(tmp_path):
    ...
    _write(rp.filled_metadata, [
        {"sample_accession": "s1", "country": "USA"},
        {"sample_accession": "s2", "country": ""},
    ])
    fails: list[str] = []
    stats = vc.check_inv3_fill(rp, fails)
...
def test_inv3_fill_passes_when_all_reach_final(tmp_path):
    ...
    _write(rp.filled_metadata, [{"sample_accession": "s1", "host": "Homo sapiens"}])
    fails: list[str] = []
    stats = vc.check_inv3_fill(rp, fails)
...
def test_amend_run_health_is_idempotent(tmp_path):
    rp = _rp(tmp_path)
    rp.run_health_md.parent.mkdir(parents=True, exist_ok=True)
    rp.run_health_md.write_text("# Run-health report\n\nbody\n")
    inv1 = {"answered": 3, "applied_pairs": 3, "n_fills": 120}
    inv3 = {"traced": 120, "in_final": 120}
    inv2 = {"disk_rows": 51, "head_rows": 50, "dropped": 0}
    vc._amend_run_health(rp, inv1, inv3, inv2)
    once = rp.run_health_md.read_text()
    assert once.count(vc._CONSERVATION_MARKER) == 1 and "VERIFIED" in once
    vc._amend_run_health(rp, inv1, inv3, inv2)
    twice = rp.run_health_md.read_text()
